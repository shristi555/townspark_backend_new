...
def is_sre(response_data):
    ...
    if (
        ...
        and ("response" in response_data or "error" in response_data)
    ):
        return True
    return False
...
class TestIssueLifecycle:
    ...
    def test_my_issues_isolation(self, user1_client, user2_client, dummy_image):
        ...
        user1_client.post(
            "/issues/create/",
            {"title": "U1", "description": "D", "uploaded_images": [dummy_image]},
            format="multipart",
        )
        ...
        user2_client.post(
            "/issues/create/",
            {"title": "U2", "description": "D", "uploaded_images": [dummy_image]},
            format="multipart",
        )
    ...
    def test_issue_creation_with_high_precision_coordinates(
        self, user1_client, dummy_image
    ):
        """Test that high precision coordinates (like from Mac browsers) are accepted."""
        print("\n--- Test: Create Issue with High Precision Coordinates ---")
        payload = {
            "title": "Issue with GPS",
            "description": "Testing high precision coordinates",
            "category": "General",
            "latitude": "37.7749295123",  # 10 decimal places
            "longitude": "-122.4194155789",  # 10 decimal places
            "uploaded_images": [dummy_image],
        }
        response = user1_client.post("/issues/create/", payload, format="multipart")
        data = response.json()
        ...
        print(f"Response: {data}")
        assert response.status_code == 201, (
            f"Expected 201, got {response.status_code}: {data}"
        )
        assert is_sre(data)
        assert data["success"] is True
        ...
        assert issue.latitude is not None
        assert issue.longitude is not None
        assert str(issue.latitude).startswith("37.7749")
        assert str(issue.longitude).startswith("-122.4194")

    def test_issue_creation_with_invalid_coordinates(self, user1_client, dummy_image):
        """Test that invalid coordinates are rejected."""
        print("\n--- Test: Create Issue with Invalid Coordinates ---")
        payload = {
            "title": "Issue with bad GPS",
            "description": "Testing invalid coordinates",
            "category": "General",
            "latitude": "200.0",  # Invalid: exceeds range
            "longitude": "-122.4194155789",
            "uploaded_images": [dummy_image],
        }
        response = user1_client.post("/issues/create/", payload, format="multipart")
        data = response.json()
